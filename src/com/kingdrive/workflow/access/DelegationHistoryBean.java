...
public class DelegationHistoryBean {
  // log4j for log
  final org.apache.log4j.Logger logger = org.apache.log4j.Logger
      .getLogger(DelegationHistoryBean.class);

  public DelegationHistoryBean() {
  }

  public void delete(int delegationHistoryId)
      throws SQLException {
    String sql = "delete from WF_DELEGATION_HISTORY where DELEGATION_HISTORY_ID=?";
    PreparedStatement st = null;
    Connection conn = null;
    try {
    	conn = ConnectionFactory.getConnection();
      st = conn.prepareStatement(sql);
      st.setInt(1, delegationHistoryId);
      st.executeUpdate();
      logger.info(sql);
  ...
  public ArrayList find(int theBegin, int theEnd)
    ...
    String sql = "select DELEGATION_HISTORY_ID, DESCRIPTION, TEMPLATE_ID, NODE_ID, SENDER, OWNER, RECEIVER, PARENT_ID, DELEGATION_TIME, START_TIME, END_TIME from WF_DELEGATION_HISTORY";
    PreparedStatement st = null;
    ResultSet rs = null;
    Connection conn = null;
    try {
  ...
  public DelegationHistoryModel findByKey(int delegationHistoryId) throws SQLException {
    DelegationHistoryModel model = new DelegationHistoryModel();
    String sql = "select DELEGATION_HISTORY_ID, DESCRIPTION, TEMPLATE_ID, NODE_ID, SENDER, OWNER, RECEIVER, PARENT_ID, DELEGATION_TIME, START_TIME, END_TIME from WF_DELEGATION_HISTORY where DELEGATION_HISTORY_ID=?";
    PreparedStatement st = null;
    ResultSet rs = null;
    Connection conn = null;
    try {
  ...
  public int insert(DelegationHistoryModel model)
      throws SQLException {
    String sql = null;
    ArrayList strList = new ArrayList();
    ArrayList valList = new ArrayList();
    if (model.getDelegationHistoryIdModifyFlag()) {
      StringUtil.makeDynaParam("DELEGATION_HISTORY_ID", model.getDelegationHistoryId(), strList,valList);
             }
    ...
    if (model.getSenderModifyFlag()) {
      StringUtil.makeDynaParam("SENDER", convertSQL(model.getSender()), strList,valList);
          }
    ...
    if (model.getReceiverModifyFlag()) {
      StringUtil.makeDynaParam("RECEIVER", convertSQL(model.getReceiver()), strList,valList);
          }
    ...
    if (model.getDelegationTimeModifyFlag()) {
      StringUtil.makeDynaParam("DELEGATION_TIME",convertSQL(model.getDelegationTime()), strList,valList);
          }
    ...
    if (model.getEndTimeModifyFlag()) {
      StringUtil.makeDynaParam("END_TIME",convertSQL(model.getEndTime()), strList,valList);
          }
    String insertString = "";
    String valsString = "";
    ...
    sql = "insert into WF_DELEGATION_HISTORY(" + insertString + ") values("
        + valsString + ")";
  ...
  public int update(DelegationHistoryModel model)
      throws SQLException {
    String sql = null;
    String updateString = "";
    ArrayList strList = new ArrayList();
    ...
    if (strList.size() == 0)
      return 0;
    valList.add(model.getDelegationHistoryId());
    int length = strList.size();
    if (length == 0) {
      updateString = null;
    } else {
    ...
    sql = "update WF_DELEGATION_HISTORY set " + updateString + " where DELEGATION_HISTORY_ID=?";
    DBHelper.executeUpdate(sql,valList.toArray());
    logger.info(sql);
    return 1;
  }

  private DelegationHistoryModel parseResultSet(ResultSet rs)
      throws SQLException {
    DelegationHistoryModel model = new DelegationHistoryModel();
    try {
      model.setDelegationHistoryId(rs.getInt("DELEGATION_HISTORY_ID"));
      if (rs.wasNull())
        model.setDelegationHistoryId(null);
    } catch (Exception e) {
      model.setDelegationHistoryId(null);
    }
